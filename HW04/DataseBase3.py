...
lemurlink="http://fiji5.ccs.neu.edu/~zerg/lemurcgi/lemur.cgi?d=3&g=p&v="
...
AVE_DOCLEN = 288 
MESG = "Results written successfully to file- Database3"
  ...
  for qry_id, terms in query_dictionary.iteritems():
     ...
     for term in terms:
          ...
          for (docid,doclen,tf) in inv_list:
            docid_score_dict[docid] += (query_term_wgt_dict[term] * (tf / (tf + 0.5 + ((1.5 * doclen)/AVE_DOCLEN))))
...
def okapimodel2(query_dictionary, query_term_wgt_dict):
  ...
  for qry_id, terms_list in query_dictionary.iteritems():
   ...
   for term in terms_list:
        ctf,df,inv_list = inverted_list(term)
        for (docid,doclen,tf) in inv_list:
           docid_score_dict[docid] += ((query_term_wgt_dict[term] * (tf / (tf + 0.5 + ((1.5 * doclen)/ AVE_DOCLEN)))) * math.log(float(NUM_DOCS/1 + df)))
   print_models(docid_score_dict, qry_id, f)       
  f.close()
  print MESG
...
def okapimodel3(query_dictionary):
  ...
  for qry_id, terms_list in query_dictionary.iteritems():
   docid_score_dict = defaultdict(lambda: 0)
   seen_dict = defaultdict(lambda: ()) # docid, term : doclen, tf, ctf, df
   docid_qryterms_dict = defaultdict(lambda: [])  # docid : [terms]
   term_ctfdf_dict = defaultdict(lambda: ())  # term : ctf, df
   for term in terms_list:
        ctf,df,inv_list = inverted_list(term)
        for (docid,doclen,tf) in inv_list:
...
def okapimodel5(query_dictionary):
  ...
  for qry_id, terms_list in query_dictionary.iteritems():
   ...
   for term in terms_list:
        ctf,df,inv_list = inverted_list(term)
        for (docid,doclen,tf) in inv_list:
            docid_score_dict[docid] += (math.log((NUM_DOCS - df + 0.5) / (df + 0.5)) * ((tf * (1.2 +1)) / (tf + (1.2 * (0.25 + (0.75 * doclen / AVE_DOCLEN))))))
   print_models(docid_score_dict, qry_id, f)
  f.close()
  print MESG
