...
lemurlink="http://fiji5.ccs.neu.edu/~zerg/lemurcgi/lemur.cgi?d=2&g=p&v="
...
AVE_DOCLEN = 288
MESG = "Results written successfully to file- Database2"
...
def okapimodel2(query_dictionary, query_term_wgt_dict):
  ...
  for qry_id, terms_list in query_dictionary.iteritems():
   ...
   for term in terms_list:
        ctf,df,inv_list = inverted_list(term)
        for (docid,doclen,tf) in inv_list:
           docid_score_dict[docid] += ((query_term_wgt_dict[term] * (tf / (tf + 0.5 + ((1.5 * doclen)/ AVE_DOCLEN)))) * math.log(float(84678/1 + df)))
   print_models(docid_score_dict, qry_id, f)       
  f.close()
  print MESG
