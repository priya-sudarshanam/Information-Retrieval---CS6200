...
#changed manually
lemurlink0="http://fiji5.ccs.neu.edu/~zerg/lemurcgi/lemur.cgi?d=3&g=p&v="
BM250_result="./bm23.txt"
...
#cleaned the 25 queries of stop words, punctuations, acronymns replaced and converted to lower case
def cleaned_queries():
  dictquery={}
  ...
  for qry_id, terms_list in query_dictionary.iteritems():
   ...
   for term in terms_list:
        ...
        for (docid,doclen,tf) in inv_list:
            docid_score_dict[docid] += (math.log((NUM_DOCS0 - df + 0.5) / (df + 0.5)) * ((tf * (2.0 +1)) / (tf + (2.0 * (0.25 + (0.75 * doclen / AVE_DOCLEN0))))))
